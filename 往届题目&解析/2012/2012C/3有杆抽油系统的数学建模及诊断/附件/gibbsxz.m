...
% rouy=rou0*rouw/(rouw+hsl*rou0-hsl*rouw);
% rouy=rou0;
f=pi*(dr/2).^2;  %截面积
a=sqrt(e/rou);
...
for g=1:jie
    ...
    for i=1:k
        ...
        dy(i)=sigma0/2+e*f(g)*sum(o1.*cos(i*2*pi/k*(1:nz))+p1.*sin(i*2*pi/k*(1:nz)));
        if g<jie
            d(i)=dy(i)-(f(g)-f(g+1))*sum(x(1:g))*rouy*9.8;
        else
            d(i)=dy(i);
        end
        dd(i)=dy(i)+f.*x*[zeros(g,1);ones(jie-g,1)]*(rou-rouy)*9.8;
    end
    uu=u;
%     plot(-uu,dd);
end
plot(-uu,dd,'LineWidth',2);
grid on;
hold on;
